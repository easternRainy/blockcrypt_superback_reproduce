...
class Header:
    ...
    def __str__(self):
        return f"{self.data_start}:{self.data_len}"
...
class EncryptedHeader:
    ...
    def decrypt(self, key, iv):
        try:
            ...
            data_start, data_len = plaintext.split(":")
...
class Block:
    def __init__(self, secrets: list[Secret], kdf, headers_len=64, data_len=None, salt=None, iv=None):
        ...
        if not salt: salt = get_random_bytes(16)
        if not iv: iv = get_random_bytes(16)
...
class EncryptedBlock:
    ...
    def _parse_data_buffer_item(self, item):
        ...
        data_enciphered = item[:-28]
        data_iv = item[-28:-16]
        data_auth_tag = item[-16:]
    ...
    def _compact(self):
        ...
        padded_header_len = header_len.to_bytes(16, byteorder="big")
